...
export const handleGenerateSetTablas = async ({ setTables, figures, withoutComodin, tables }: handleGenerateSetTablasProps) => {
    ...
    const localTables = await generateTables(comodin,size,qyTables,figures,withoutComodin,tables).then((res) => {
        return res;
    });
...
const generateTables = async (comodin: number, size: number, qyTables: number, figures: Figure[], withoutComodin: boolean, tables: Table[]) => {
    const localTables = [];
...
const calculateMinLengthArray = ({ size, withoutComodin }: CalculateMinLengthArrayProps) => {
...
const updateMinMaxNum = ({oldArray, maxNum, minNum} : updateMinMaxNumProps) => {
    oldArray.map((f) => {
        if (f.id > maxNum) {
        maxNum = f.id;
        }
